...
# -----------------------
# 7. Hybrid Chat System (Final API Integration)
# -----------------------
st.divider()
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        sys_msg = (
            ...
            f"If the user wants an image/photo, start exactly with 'GENERATE_IMAGE: [English Prompt]' "
            f"and then explain in Sinhala. Today: {datetime.date.today()}. User: {user_info['full_name']}."
        )
        ...
        try:
            ...
            # --- Authenticated Image Generation Logic ---
            final_img_url = None
            if "GENERATE_IMAGE:" in full_res:
                try:
                    ...
                    # API Key එක Query Parameter එකක් ලෙස එක් කිරීම (Documentation එකට අනුව)
                    auth_param = f"&key={POLLINATIONS_KEY}" if POLLINATIONS_KEY else ""
                    # Image Lab එකේ පාවිච්චි කරන flux මාදිලියම මෙතැනටත් එකතු කළා
                    ...
                    st.image(final_img_url, caption="Generated by Alpha AI", use_container_width=True)
            ...
            msg_data = {"role": "assistant", "content": full_res}
            if final_img_url: msg_data["image_url"] = final_img_url
            st.session_state.messages.append(msg_data)
            
        except Exception as e: st.error(f"Alpha Core Error: {e}")
