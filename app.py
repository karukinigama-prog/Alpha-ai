...
# Updated InferenceClient with provider for Image Generation
...
with tab_img:
    with st.container():
        ...
        if col2.button("Generate Photo"):
            if img_p:
                with st.spinner("Alpha is painting with Z-Image-Turbo... 🖌️"):
                    try:
                        # Updated to use Tongyi-MAI model via fal-ai provider
...
if user_input:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Alpha is thinking..."):
            ...
            sys_msg = f"You are Alpha AI, a heartfelt assistant created by Hasith. Respond warmly. Creator: Hasith."
            try:
